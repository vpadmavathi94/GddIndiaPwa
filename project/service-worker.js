...
self.addEventListener('install', (event) => {
  ...
  //simple version of newtworkFirst cache
//   workboxSW.router.registerRoute(/(.*)\/articles\/(.*)html/,
...
workboxSW.router.registerRoute(/(.*)((index|\/articles\/)(.*)html)|(.*)\/$/, args => {
    return workboxSW.strategies.networkFirst().handle(args).then(response => {
        //returns offline.html if the network doesnt return a response
        // The networkFirst().handle() method returns a promise that resolves with
        //  either the network response (if online) or a response from the cache (if offline). 
        // If the network is unavailable and the requested resource,
        //  is not found in the cache (because the user never visited the corresponding page), 
